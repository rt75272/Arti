...
# and `pip install artifact` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "artifact", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n artifact -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "artifact", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "artifact", "YOUR_CONSOLE_SCRIPT"]
